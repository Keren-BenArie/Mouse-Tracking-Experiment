...
jsPsych.plugins['survey-multi-choice-text'] = (function() {
  var plugin = {};

  plugin.info = {
    name: 'survey-multi-choice-text',
    description: '',
    parameters: {
      questions: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
        ...
        trial_form.innerHTML += '<div style="margin-left: 100px; margin-right: 100px;" id="'+_join(plugin_id_name, i)+'" class="'+question_classes.join(' ')+'"></div>';
...
//        display_element.querySelector(question_selector + " p").innerHTML += "<span class='required'>*</span>";
      ...
      // add questions
      var html='<div style="margin-left: 100px; margin-right: 100px">';
      for (var i = 0; i < trial.text_questions.length; i++) {
          ...
          html += '<p class="jspsych-survey-text">' + trial.text_questions[i].prompt + '</p>';
          if(trial.text_questions[i].rows == 1){
              html += '<input type="text" name="#jspsych-survey-text-response-' + i + '" size="'+trial.text_questions[i].columns+'" value="'+trial.text_questions[i].value+'"></input>';
          } else {
              html += '<textarea name="#jspsych-survey-text-response-' + i + '" cols="' + trial.text_questions[i].columns + '" rows="' + trial.text_questions[i].rows + '">'+trial.text_questions[i].value+'</textarea>';
          }
          html += '</div>';
      }
      html += '</div>'
      trial_form.innerHTML += html;
    // add submit button
    ...
    trial_form.addEventListener('submit', function(event) {
      ...
      for(var i=0; i<matches.length; i++){
        ...
        var text_matches = display_element.querySelectorAll('div.jspsych-survey-text-question');
        for(var index=0; index<text_matches.length; index++){
            var id = "T" + index;
            var val = text_matches[index].querySelector('textarea, input').value;
            var obje = {};
            obje[id] = val;
            Object.assign(question_data, obje);
            }
