...
jsPsych.plugins['survey-multi-choice'] = (function() {
    ...
    parameters: {
      questions: {
        ...
        nested: {
          ...
          recommended: {type: jsPsych.plugins.parameterType.STRING,
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var cssstr = ".jspsych-survey-multi-choice-question { margin-top: 2em; margin-bottom: 2em; text-align: left; }" +
      ...
      ".jspsych-survey-multi-choice-horizontal .jspsych-survey-multi-choice-text {  text-align: center !important;}"+
      ".jspsych-survey-multi-choice-option { line-height: 2; }"+
      ".jspsych-survey-multi-choice-horizontal .jspsych-survey-multi-choice-option {  display: inline-block; text-align: center !important;  margin-left: 1em;  margin-right: 1em;  vertical-align: top;}"+
    ...
    for (var i = 0; i < trial.questions.length; i++) {
        ...
        trial_form.innerHTML += '<div style="margin-left: 100px; margin-right: 100px; text-align: ' + (trial.questions[i].horizontal ? "center":"left") +'" id="'+_join(plugin_id_name, i)+'" class="'+question_classes.join(' ')+'"></div>';
...
//        display_element.querySelector(question_selector + " p").innerHTML += "<span class='required'>*</span>";
      ...
      if (trial.questions[i].recommended != null) {
          var re = document.createElement('p');
          re.setAttribute('style', "color: red; visibility: hidden;");
          re.setAttribute('id', plugin_id_name + '-recommendation-' + i);
          re.innerHTML = trial.questions[i].recommended;
          document.querySelector(question_selector).appendChild(re);
      }
...
// add submit button
        if (!trial.response_ends_trial) {
            trial_form.innerHTML += '<p style="margin-left: 100px; margin-right: 100px; text-align: center;"><input type="submit" id="' + plugin_id_name + '-next" class="' + plugin_id_name + ' jspsych-btn"' + (trial.button_label ? ' value="' + trial.button_label + '"' : '') + '></input></p>';
            trial_form.addEventListener('submit', submitForm);
        }
        else {
            inputs = display_element.querySelectorAll("input[type=radio]");
      ...
      function submitForm(event) {
          ...
          for (var i = 0; i < matches.length; i++) {
              ...
              if (match.querySelector("input[type=radio]:checked") !== null) {
                  ...
                  var val = "";
                  if (!recommended && trial.questions[i].recommended != null) {
                      missingRecommended = true;
                      document.getElementById(plugin_id_name + '-recommendation-' + i).style.visibility = 'visible';
                  }
              }
              var obje = {};
              obje[id] = val;
